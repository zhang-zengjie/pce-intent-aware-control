...
class PCEMICPSolver:
    ...
    :param pce_0:              A ``(n,1)`` numpy matrix describing the initial state.
    ...
    def __init__(self, spec, sys, pce_0, T, M=1000, robustness_cost=True,
        ...
        self.x = self.model.addMVar((self.sys.Ab.shape[0], self.sys.Ab.shape[1], self.T), lb=-float('inf'), name='x')
        self.u = self.model.addMVar((2, self.T), lb=-float('inf'), name='u')
    ...
    def AddDynamicsConstraints(self):
        # Initial condition
        self.model.addConstr(self.x[:][:][0] == self.pce_0, name='init_state')

        # Dynamics
        for t in range(self.T - 1):

            self.model.addConstr(
                self.x[:][:][t+1] == self.sys.pce_model(self.x[:][:][t], self.u[:][t]),
                name='regular_state'
            )
