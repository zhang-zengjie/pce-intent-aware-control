import numpy as np
import numpoly
from commons import eta, gen_pce_coefficients, bicycle_forward, bicycle_linear_forward
...
np.random.seed(0)
nodes = eta.sample([M, ])
mc_samples = np.array([bicycle_forward(N, zeta_0, u, node[0], node[1]) for node in nodes.T])
mc_samples_linear = np.array([bicycle_linear_forward(N, zeta_0, u, node[0], node[1]) for node in nodes.T])
