...
intent = cp.Trunc(cp.Normal(1, 1e-5), lower=1-1e-5, upper=1+1e-5)       # Random variable for intent uncertainty
...
bicycle_pce = BicycleModel(x0, nodes[:, 0], Ts, B, pce=True, name="pce")
bicycle_pce.update_initial(x0)
x_hat = bicycle_pce.predict_pce(u, N)

bicycle = BicycleModel(x0, nodes[:, 0], Ts, name="nonlinear")
...
bicycle_linear = BicycleModel(x0, nodes[:, 0], Ts, name="linear")
...
pyplot.plot(np.linspace(0, 1, N+1), np.std(mc_samples, 0)[0])
pyplot.plot(np.linspace(0, 1, N+1), np.std(mc_samples_linear, 0)[0])
